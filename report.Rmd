...
This is a rating prediction project based off of the [Netflix Prize](https://en.wikipedia.org/wiki/Netflix_Prize), in which participants were challenged to surpass Netflix's movie rating prediction accuracy. The goal in the competition and in this project was to minimize the prediction error, specifically the root mean-squared error (RMSE). I implemented a variety of increasingly complex models on a subset of the MovieLens dataset and observe significant decreases in RMSE.
...
The first three models - simple average, movie and user effects, and regularized movie and user effects - are implementations of the models demonstrated in the *edx Data Science: Machine Learning* course. The last three are exploratory and use the recommenderlab package. The first three models were tuned only on the training set as is best practice. The final three, because they were exploratory and outside the requirements of this project, were lightly tuned on the test set in the interests of time. 
...
The values of the $\lambda$'s were determined by minimizing RMSE on the training set with cross-validation ($k = 10$). When creating the folds, users or movies who appreared in the test folds but not their respecting training sets were removed to the training sets, just as was done with the actual test set during data preparation. After applying the model to each fold, the mean of the 10 RMSE's was calculated and minimized with Nelder-Mead, R's default optimization algorithm.
...
movie_effects %>% add_column(time = "Before") %>%
  ...
  geom_point(alpha = .5) +
  facet_grid(.~time) + 
  theme_few() + labs(x = "Number of Ratings", y = "Movie Effect")
```
 
 
## Personalization with Recommender Lab
 
Recommending the best rated movies is valuable in itself, but ideally recommendations are personalized. The recommenderlab package enabled some personalization and reduced RMSE substantially. Reminder: tuning for these models took place on the test set for expediency, and should be interpreted with a grain of salt.

### Tuning and RMSEs

**UBCF:** UBCF was the only algorithm that did not significantly outperform MUFR. It's results were improved by calculating distance between users with Pearson's rather than Cosine's. UBCF is commonly used and known as a top recommendation algorithm, but it is more memory and computationally intensive than SVD or IBCF when making predictions. It took much longer to run and likely needs to be scaled up to be effective.
...
**SVD:** The SVD model also outperformed MUFR. The only tuning that took place was to increase $k$ from $10$ to $20$, where $k$ is the number of most significant vector pairs used to estimate the residual. Increasing $k$ beyond that yielded some results but not much, suggesting that the higher-numbered primary components become less relevant and represent noise rather than actual user properties.

### Changed Recommendations

Like with regularization, we can also see a change in movie rankings in addition to RMSE. The algorithm now will recommend different top movies (from the top 1000 movies) for each person from the list of movies they have not rated. In tables 5 and 6, the top five recommendations for two users are arranged by algorithm. Most of the recommendations are familiar from the overall top 20 movies. The Recommender Lab algorithms reorder recommendations but largely respect the overall ranking of movies - in another iteration, this could be changed by weighting the predicted residual higher than the movie effect. IBCF and SVD seem to introduce much more variation than UBCF in the examples.

 ```{r, echo = FALSE}
  top_user_recs <- function(n_recs, whichUser){
    all_ratings_200 %>% filter(userId == whichUser) %>% 
    group_by(model) %>% 
    top_n(n_recs, wt = rating) %>% 
    mutate(Rank = rank(-rating)) %>% 
    ungroup() %>% 
    left_join(movies, by = "movieId") %>% 
    select(title, model, Rank) %>% 
    mutate(title = str_trunc(str_remove(title, "\\(\\d{4}\\)"), 20)) %>%
    spread(model, title) %>% 
    select(Rank, MUFR, UBCF, IBCF, SVD) %>%
